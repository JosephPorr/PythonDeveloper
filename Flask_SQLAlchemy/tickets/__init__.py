...
# This is not the finalized __init__, and will add more variables.

import os

from flask import Flask, abort, redirect, render_template, url_for

def create_app(test_config=None):  # this is called a "factory create app in flask"
    app = Flask(__name__)
    app.config.from_mapping(
        SECRET_KEY=os.environ.get('SECRET_KEY', default='dev'),
    )

    if test_config is None: # Load configuration from config.py
        app.config.from_pyfile('config.py', silent=True)
    else:
        app.config.from_mapping(test_config)  # can be used for automatged testing/mapping
